...
export function parseState(json: string): MachineState {
  ...
  if (raw?.offsets && typeof raw.offsets === "object") { for (const [k, v] of Object.entries(raw.offsets)) { if (!/^G5[4-9](\.\d+)?$/.test(k)) throw new Error("Offset '" + k + "' is not a supported work offset (G54-G59.x)."); if (!isVec(v)) throw new Error("Offset " + k + " must be numeric x/y/z."); offsets[k] = v; } }
  const tools: Record<string, { diameter?: number; length?: number }> = {};
  if (raw?.tools && typeof raw.tools === "object") { for (const [k, v] of Object.entries(raw.tools)) { if (!/^T\d+$/.test(k)) throw new Error("Tool '" + k + "' must be named T<n> (e.g. T1 or T01)."); const t: any = v ?? {}; const normalizedKey = "T" + k.slice(1).padStart(2, "0");
